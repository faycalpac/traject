...
trajeR.CNORM <- function(Y, A, X, TCOV, ng, nx, n, nbeta, nw, ntheta, period, degre, theta, beta, sigma, delta, pi, Method, ssigma,
  ...
  if (Method == "L") {
    ...
    # valeur initiale pour la methode CEM
    if (is.null(paraminit)) {
      sigma <- rep(stats::sd(Y), ng)
      if (nx == 1) {
        paraminitEM <- c(pi[1:(ng - 1)], unlist(beta), sigma, unlist(delta))
    ...
    if (max(Y) < ymax & min(Y) > ymin) {
      if (ssigma == FALSE) {
        param <- CEM_cpp(paraminitEM, ng, nx, nbeta, n, A, Y, X, ymin, ymax, TCOV,
                         nw, itermax, EMIRLS, refgr)
      } else {
        param <- CEMSigmaunique_cpp(paraminitEM, ng, nx, nbeta, n, A, Y, X, ymin,
                                    ymax, TCOV, nw, itermax, EMIRLS, refgr)
      }
    } else {
      if (ssigma == TRUE) {
        param <- CEMCensoredSigmaunique_cpp(paraminitEM, ng, nx, nbeta, n, A, Y,
                                            X, ymin, ymax, TCOV, nw, itermax, EMIRLS,
                                            refgr)
      } else {
        param <- CEMCensored_cpp(paraminitEM, ng, nx, nbeta, n, A, Y, X, ymin, ymax,
                                 TCOV, nw, itermax, EMIRLS, refgr)
      }
    }
    if (hessian == TRUE) {
